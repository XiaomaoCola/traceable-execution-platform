...
from backend.app.agents.fortune.prompts import SYSTEM_PROMPT
from backend.app.agents.fortune.tools import TOOLS, execute_tool
...
async def generate(message: str) -> AsyncGenerator[str, None]:
    """算命 Agent 主流程，返回 SSE 事件流。

    SSE 事件类型：
      tool_start  — Agent 开始调用某工具
      tool_result — 工具返回结果
      text_chunk  — 最终运势报告的逐字流
      error       — 出错信息
      done        — 流结束
    """
    messages = [
